...
#Supress rdkit warning messages
...
desc_sets_filename = ['desc_2D_3D_all.csv', 'desc_2D_all.csv']

# Step 1: Calculate and save the descriptor sets
db = read_db(path_to_database, name_target=target_name, smiles_col='smiles', debugging=False)
...
train_indices = np.load(f'splits/train_ids_split1.npy', allow_pickle=True)
test_indices = np.load(f'splits/test_ids_split1.npy', allow_pickle=True)
...
for desc_set in desc_sets_filename:
    ...
    train_df = df.loc[df.index.intersection(train_indices)]
    test_df = df.loc[df.index.intersection(test_indices)]
    ...
    for model_name in ["SVR", "RandomForestRegressor", "KNeighborsRegressor", "HistGradientBoostingRegressor", "MLPRegressor"]:
        ...
        study.optimize(lambda trial: objective(trial, model_name, X_train, y_train), n_trials=25, n_jobs=-1)

        best_params = study.best_params
        print(f"Best params for {model_name}: {best_params}")

        # Save best hyperparameters
        with open(f"best_hyperparameters/best_params_1.txt", "a") as f:
        ...
        model_filename = f"models/split1_{dataset_name}_{model_name}_best.pkl"
        ...
        with open(f"predictions/metrics_split1.txt", 'a') as f:
    ...
    results_filename = f"predictions/split1_{dataset_name}_best_predictions.csv"
