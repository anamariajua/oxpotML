...
def train_model(
    ...
    for i, dataset in enumerate(training_dataloader):
        ...
        nn_input = (node_mat.to(device), adj_mat.to(device))
        ...
        #Check if prediction is non NaN
        if nn_prediction is None:
            print(f"NaN detected in prediction of molecule with ID:{id}")
            continue
...
def predict_gcn(model,test_dataloader,standardizer,device,max_atoms,node_vec_len):
    """
