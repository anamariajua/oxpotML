"""Nested cross-validation loop for the enriched Graph Convolutional Nueral Network model"""
import numpy as np
import pandas as pd
import torch
...
n_epochs = 50
NUM_GPUS = 4
best_params = {'hidden_nodes': 177, 'n_conv_layers': 1, 
               'n_hidden_layers': 2, 'learning_rate': 0.0015623011393125334, 
               'p_dropout': 0.4028720333169325}
...
for split in range(10):
    ...
    torch.save(model.state_dict(), f"models/enriched_GCN_split{split+1}.pth")
    ...
    if split == 0:
        ...
        results_filename = "predictions/neural_fp_enriched_GCN.csv"
    ...
    with open(f"predictions/metrics_enriched_GCN.txt", 'a') as f:
        f.write(f"Metrics for enriched GCN model: split {split+1}\n")
...
# Save predictions
results_filename = f"predictions/enriched_GCN_predictions.csv"
